...
mg[is.na(mg)] <- 0
# See number of counts per column and get rid of barcodes with <500 counts.
# For Illumina sequencing it is recommended to remove barcodes with < 2000 counts,
# but ONT reads are much longer (1.5 - 1.8 kb in our experience)
table(colSums(mg[2:ncol(mg)]))
keep.barcodes <- colSums(mg[2:ncol(mg)]) > 500
...
sampleinfo$group <- factor(c("pBICA", rep("pBIC10",3), rep("pBIC",2),rep("pBI",2), 
...
counts.aldex <- counts[,c(2:4,12,5:6,13:14)]
conds <- c(rep("pBIC10", 4), rep("pBIC", 4))
# Run ALDEx2:
x.all <- ALDEx2::aldex(counts.aldex, conds, mc.samples=128, test="t", effect=TRUE,
