...
# Following recommendations from https://doi.org/10.1038/s41467-022-28034-z, we compare
# results using two different methods: limma-voom (with TMMwsp normalization) and ALDEx2.
...
counts.cpm <- cpm(dge, log = F)
barplot(colSums(counts.cpm), names=colnames(counts.cpm),las=2, cex.names = 0.6)
...
#Quality check with simple visualizations
#Heatmap of OTUS with most variation:
var_otus <- apply(log.counts.cpm, 1, var)
select_var <- names(sort(var_otus, decreasing=TRUE))[1:100]
highly_variable_lcpm <- log.counts.cpm[select_var,]
...
design
#'The voom transform creates an EList object and shows a mean-variance plot:
v <- voom(dge, design, plot = TRUE)
#'See logCPM:
head(v$E)
#'Now we can compare unnormalised logCPM with voom-normalised
par(mfrow=c(1,2))
boxplot(log.counts.cpm, xlab="", ylab="Log2 CPM",las=1, cex.axis = 0.7, main="Unnormalised logCPM")
abline(h=median(log.counts.cpm),col="blue")
...
fit<-lmFit(v)
names(fit)
#'and create contrast matrix (for one comparison)
cont.matrix <- makeContrasts(bic_bic10=pBIC - pBIC10, 
                             levels=design)
cont.matrix
#'Run statistics between all contrats:
fit.cont <- contrasts.fit(fit, cont.matrix)
fit.cont <- eBayes(fit.cont)
#'and correct for multiple testing:
sum.fit <- decideTests(fit.cont, p.value = 0.05, lfc = 1) #'adjusted p-value
summary(sum.fit)
#'
#'Now get top DA OTUs for the contrast with adjusted p-value and logFC.
...
tail(result.limma)
#Check heatmap
heatgenes <- (log.counts.cpm[rownames(result.limma),])
pheatmap::pheatmap(heatgenes, scale = "row", fontsize_row = 8, annotation_col = sampleinfo, legend = T)
...
# ALDEx2
#'-----------
# Retain only samples from the two groups to be compared
counts.aldex <- counts[,c(1:3,11,4:5,12:13)]
conds <- c(rep("pBIC10", 4), rep("pBIC", 4))
x.all <- ALDEx2::aldex(counts.aldex, conds, mc.samples=16, test="t", effect=TRUE,
